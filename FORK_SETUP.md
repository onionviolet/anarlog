...
0. **DONE 2026-09-04: diarization is untied from Parakeet, so speaker labels and Mandarin now coexist.** Upstream gave labels **or** Chinese and never both, because `diarize_samples` refused every model but `ParakeetBatch` and Parakeet carries no Chinese.
   ...
   **The prediction was right and the guessed fix was wrong.** The Rust gate was indeed arbitrary, since the diarizer clusters speaker embeddings from raw samples and never reads a word. But widening that check alone would have failed: the Swift bridge loads `Community1DiarizationPipeline` **inside** the `.parakeetBatch` case and stores it in that enum variant, so `asDiarizationPipeline()` succeeds for exactly one model by construction. **[grammar] A gate you can see is rarely the only one.** Read the layer underneath before assuming a check is the whole restriction.

   **What the fix actually took:** the diarizer became a standalone lazily-loaded property on the bridge, shared behind one in-flight task so two recordings finishing together do not each pull the model, and `diarizeAudioJSON` now validates the model id without using it to reach the pipeline. Then **Omnilingual** was added to `SELECTABLE`, because it is the only Soniqo model that transcribes Chinese and it was already implemented and downloadable behind that list alone. **That is the fork's original 2026-06-24 goal, arrived at from the opposite direction and for a better reason.**

   **The honest limit.** This covers the Soniqo path only. **Apple Speech and the Argmax models route through different crates and still produce unlabelled text**, so the model to pick for labelled Mandarin is Omnilingual, not Apple Speech. Whether the diarizer's accuracy holds on Mandarin audio is untested here; the pipeline is acoustic, so there is no reason in principle for it to differ, and that is a reason rather than a measurement.
